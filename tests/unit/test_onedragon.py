...
def test_classic_preflight_requires_official_resource_configs(tmp_path: Path):
    executable = tmp_path / ZZZ_ONEDRAGON_CLASSIC_NAME
    executable.touch()
    (tmp_path / "resources" / "config").mkdir(parents=True)
    (tmp_path / "resources" / "config" / "project.yml").write_text("name: zzz", encoding="utf-8")
    (tmp_path / "resources" / "config" / "repository.yml").write_text("name: repo", encoding="utf-8")
    report = run_onedragon_preflight(
        {
            "config_version": 1,
            "executable_path": str(executable),
